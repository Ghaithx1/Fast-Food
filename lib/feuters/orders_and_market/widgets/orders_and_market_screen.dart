...
class OrdersAndMarketScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: FloatingActionButton(onPressed: ()async => await launch(
         "https://wa.me/${905519384959}?text=$foodtoMarket")),
      body: SafeArea(
        child: Padding(
          padding: const EdgeInsets.symmetric(horizontal: 25.0, vertical: 20),
          child: BlocBuilder<OrdersAndMarketCubit, OrdersAndMarketState>(
            builder: (context, state) {
              return state.when(initial: (){return Text('Loading');}, loaded: (foods, money,currentType,foodToMarket) {
                      ...
                      nameText(
                        ...
                        ontap:  () {
                          ...
                          return FoodTypeDialog(changeTheType: (type){
                            BlocProvider.of<OrdersAndMarketCubit>(context).filterTheDataByType(type);
