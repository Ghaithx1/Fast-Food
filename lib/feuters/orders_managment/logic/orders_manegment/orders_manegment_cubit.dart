...
class OrdersManegmentCubit extends Cubit<OrdersManegmentState> {
  ...
  OrdersManegmentCubit(this.ordersManagmentRepo)
      : super(OrdersManegmentState.initial()) {}
  List<User> users = [];
  ...
  void fetchUsers() async {
    //First thing when anitializing the cubit it fetch the users
    emit(OrdersManegmentState.Loading());
    index = -1;
    List<User> respnse =
        await ordersManagmentRepo.GetTheUsersHavenWriteFoodYet();
  ...
  _sentUserAndFoodToScreenAndHandleIfItEmpty(User? user) {
    emit(OrdersManegmentState.Loaded(
        users.isEmpty
            ? User(name: "Add user", money: 0, taken: false)
            : users[index],
        usersWithFood,
        0));
  }
  ...
  void fetchUsersWithoutAction() async {
    ...
    users = respnse;
    emit(OrdersManegmentState.Loaded(
        users[index], usersWithFood, users[index].money));
  }
  ...
  void getFoodbyIdAndAddFoodOrRemove(
      User user, int food_id, bool AddOrRemove) async {
    print('The food is');
  ...
  void addFoodForUser(User user, Food food) async {
    var respon =
        await ordersManagmentRepo.checkIfFoodForUserExistsAndAdd(user, food);
